...
class NavigationViewModel @Inject constructor() : ViewModel() {
    ...
    fun processIntent(intent: NavigationIntent) {
        _state.update { currentState -> // currentState is of type NavigationUIState
            when (intent) {
                ...
                is NavigationIntent.GoToPlayer -> {
                    // Navigate to Home.
                    ...
                    if (currentState.backStack.lastOrNull() != Screen.Player) {
                        currentState.copy(backStack = currentState.backStack + Screen.Player)
                    } else {
                        currentState // No change
                    }
                }
    ...
    /**
     * Goes to the Player screen by sending a [NavigationIntent.GoToPlayer].
     * (Current implementation adds Home to stack if not already at top).
     */
    fun goToPlayer() {
        processIntent(NavigationIntent.GoToPlayer)
    }
